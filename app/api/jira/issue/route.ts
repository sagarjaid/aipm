  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json({ error: `Failed to fetch issue: ${errorText}` }, { status: response.status })
      }
      ...
      if (fetchError.name === "AbortError") {
        return NextResponse.json({ error: "Request timeout while fetching issue" }, { status: 408 })
      }
