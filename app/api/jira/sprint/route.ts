...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isNaN(Number(boardId))) {
      return NextResponse.json(
        { error: `Invalid board ID: ${boardId}` },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      if (!testResponse.ok) {
        ...
        return NextResponse.json(
          { error: `Authentication failed: ${testError}` },
    ...
    try {
      ...
      const activeSprintsResponse = await makeJiraRequest(
        activeSprintsUrl,
        auth,
      );
      ...
      if (activeSprintsResponse.ok) {
        ...
        console.log(
          `Found ${activeSprintsData.values?.length || 0} active sprints`,
        );
    ...
    if (!sprintData) {
      try {
        ...
        if (allSprintsResponse.ok) {
          ...
          console.log(
            `Found ${allSprintsData.values?.length || 0} total sprints`,
          );

          if (allSprintsData.values && allSprintsData.values.length > 0) {
            // Find the most recent sprint
            const sortedSprints = allSprintsData.values.sort(
              (a: any, b: any) => {
            ...
            console.log(
              `Using most recent sprint: ${sprintData.name} (${sprintData.state})`,
            );
          }
        } else {
    ...
    if (sprintData) {
      try {
        console.log(
          `Attempting to fetch issues for sprint ${sprintData.id}...`,
        );
        const sprintIssuesUrl = `https://${cleanDomain}/rest/agile/1.0/sprint/${sprintData.id}/issue`;
        const sprintIssuesResponse = await makeJiraRequest(
          sprintIssuesUrl,
          auth,
        );
