  ...
  try {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`Failed to get comments: ${response.status} - ${errorText}`)
        return NextResponse.json({ error: `Failed to get comments: ${errorText}` }, { status: response.status })
      }
      ...
      if (fetchError.name === "AbortError") {
        return NextResponse.json({ error: "Request timeout while fetching comments" }, { status: 408 })
      }
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      if (fetchError.name === "AbortError") {
        return NextResponse.json({ error: "Request timeout while adding comment" }, { status: 408 })
      }
