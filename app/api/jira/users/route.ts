...
export async function POST(request: NextRequest) {
  try {
    const { domain, email, apiToken, projectKey, issueKey } = await request.json()
    ...
    if (!projectKey && !issueKey) {
      return NextResponse.json({ error: "Either projectKey or issueKey must be provided" }, { status: 400 })
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json({ error: `Failed to get users: ${errorText}` }, { status: response.status })
      }
      ...
      if (fetchError.name === "AbortError") {
        return NextResponse.json({ error: "Request timeout while fetching users" }, { status: 408 })
      }
